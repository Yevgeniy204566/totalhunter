...
```powershell
# 0. git status ЧИСТ (нет modified/untracked файлов кода, конфигов, профилей)
git status --short                              # → пусто (или только заведомый мусор вне репо-кода)
# Сборка идёт с ДИСКА (Nuitka пакует рабочую папку, не git checkout) — если тут есть
# незакоммиченные правки, exe разойдётся с git НАВСЕГДА (см. случай v1.9.0/1.9.1/1.9.2
# 2026-09-08/10: version.py на диске был "1.9.1", в git — "1.8.18", бампы 1.8.19…1.9.1
# никогда не коммитились, хотя релизы фактически выходили). Коммитить ВСЁ нужное
# для релиза ДО сборки, не постфактум.
...
- **🔒 РАБОТА С ДОКУМЕНТАМИ (спеки/планы) — `docs/РАБОТА-С-ДОКУМЕНТАМИ.md`**, обязательный протокол для
  ...
  Review → Test Matrix → Self-Audit → Внешнее ревью (Stage 8 — ТОЛЬКО по прямому запросу пользователя, агентов
  без запроса нет) → Final Gate → User Review), спеки/планы пишет сам Claude в основном диалоге, **лимит раундов ОТМЕНЁН (2026-09-18)**: правки идут
  столько раундов, сколько нужно, пока внешние ревьюеры (GPT/Gemini, прогоняет владелец) не одобрят; после каждого
  раунда показывать документ владельцу снова, без `AskUserQuestion` по счётчику; scope ради замечаний не расширять. Читать документ целиком перед КАЖДОЙ новой спекой/планом.
...
- **🔒🔒🔒🔒 НИКОМУ НЕ ДОВЕРЯТЬ СЛЕПО — ПЕРЕПРОВЕРЯТЬ ВСЁ (золотое правило, 2026-09-18, касается ВСЕГО проекта):** ревью GPT/Gemini, отчёты агентов, пересказы, старые документы и заметки, собственная память о коде — это ГИПОТЕЗЫ, а не факты. Каждое утверждение проверять лично по коду и по документам (открыть файл, прочитать, при необходимости запустить). Ошибаться могут все — не по злому умыслу, а потому что ревьюеры не знают код так, как знает его Claude. Особенно внешние ревьюеры: их замечание принимается, только если подтверждено кодом/спекой; неверное — отклонять с объяснением. **Последнее слово всегда за Claude** (после личной проверки), а решения по числам/дизайну/scope — за владельцем.
- **🔒🔒🔒🔒 АГЕНТЫ — ТОЛЬКО С ЯВНОГО ОДОБРЕНИЯ ПОЛЬЗОВАТЕЛЯ (золотое правило, 2026-09-18):** запускать субагентов (Agent/Task, `fork`, SDD-исполнители, Stage 8-ревьюеры, Explore и любые другие) ЗАПРЕЩЕНО без явного одобрения/разрешения пользователя в текущей сессии. МОЖНО и допустимо ПРЕДЛОЖИТЬ агента (для написания кода, ревью или другой работы) с оценкой стоимости в токенах — запуск только после явного «да». Без одобрения всё — спеки, планы, код, ревью, поиск по коду — делаю САМ в основном диалоге. Общее «работаем по полной» или согласие на задачу одобрением агента НЕ является. Запрет («без агента»), произнесённый пользователем в сессии, действует до конца сессии, повторно не предлагать. Причина: агент читает контекст заново и сжигает сотни тысяч токенов (разделение спеки Биржи 2.0 = ~230 тыс.). Правило перекрывает любые скилы (`subagent-driven-development`, `dispatching-parallel-agents`, `writing-plans` и т.п.), которые предлагают диспатч.
