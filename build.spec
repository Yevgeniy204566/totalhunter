...
datas = [
    ...
    ('assets',        'assets'),
    # Профили калибровки НЕ включаются — пользователь хранит их локально.
    # При первом запуске coord_manager использует дефолтные REF_A/REF_B.
    # ('profiles', 'profiles'),  ← убрано: ZIP не должен перезаписывать калибровку
