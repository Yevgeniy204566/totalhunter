...
1. Распарсить form-поля: sale_id, email, price (МИНОРНЫЕ ЕДИНИЦЫ — копейки;
   подтверждено реальным вебхуком: price=100 при цене 1 грн), product_id,
   quantity, test. Все значения из `request.form()` — `str` (Starlette/FastAPI
   не приводит типы form-данных сама) — `price_kopecks = int(price)` сразу
   здесь, дальше по потоку использовать только `price_kopecks` (число), не
   сырую строку. `sale_id` на этом шаге — только сырой
   ...
   Валидация ДО любой работы с БД и сетью: отсутствие
   `sale_id`/`email`/`price`/`product_id`, `sale_id` длиннее 50 символов
   (столько же, сколько колонка `BlackSeaSale.sale_id`) или `price`, не
   приводящийся к `int` → `HTTPException(400)`, ничего не логировать как
   продажу. Единственная ветка потока, отвечающая не 200 при синтаксически
   осмысленном запросе — остальные ветки ниже всегда 200, даже при отказе.
...
4. Позвать GET /api/v2/sales/{sale_id}?access_token=... (см. рецепт в
   ...
   **Плюс сверить email/price/product_id API-ответа (`sale.email`,
   ...
   КАЖДАЯ покупка проваливала бы сверку. **Если ответ содержит собственный
   идентификатор продажи (`sale.id`) — он тоже обязан совпадать с
   `sale_id`, который был подставлен в URL запроса** (дешёвая
   defense-in-depth проверка целостности ответа, на случай прокси/кеша
   между нами и BlackSea — не единственная защита, но не помешает).
   ...
   - **Отсутствующее или не того типа обязательное поле в ответе API
     (`paid`/`chargedback`/`refunded`/`email`/`price`/`product_id`
     отсутствует, `null`, или не того типа — например `paid` не `bool`) →
     считать верификацию ПРОВАЛЕННОЙ (fail-closed), 200 OK, лог ERROR, НЕ
     начислять.** Не пытаться угадать смысл нестандартного ответа — API
     BlackSea внешний, недокументированный по этому эндпоинту (см. память),
     трактовать его отклонение от ожидаемой формы как отказ, не как
     разрешение.
   ...
   - **`quantity` из тела вебхука не равен `"1"`** (сравнение строки как
     есть, не `int(quantity)` — не принимать `"01"`/`"1.0"` как эквивалент)
     → 200 OK, не начислять автоматически, тот же алерт на ручной разбор,
     что при ненайденном email. Отсутствие поля трактуется как `"1"` с
     логом WARNING. Why: умножать пакет на количество — несогласованное
     бизнес-правило, начислить один пакет за оплаченные несколько означало
     бы недодать покупателю; поле берётся из недоверенного тела, но это
     допустимо ровно потому, что оно работает только как консервативный
     гейт «уйти на ручной разбор» — подделка не может увеличить начисление,
     может только отправить свою же покупку на лишнюю проверку.
5. Найти User по **`sale.email` из API-ответа** (НЕ по `email` из тела
   ...
   Загружать с `.with_for_update()` (тот же приём, что `payments.py:192`
   для `Order`) и держать лок до `commit()` в п.9 — `user.credits += credits`
   в п.7 это read-modify-write; без лока две одновременные оплаченные
   продажи одного пользователя (два разных `sale_id`) могут прочитать один
   и тот же стартовый баланс и потерять одно из начислений.
6. **Атомарная точка идемпотентности.** Шаги 2 и 5 уже выполнили SELECT на
   ...
   **Про `await db.flush()`:** эмпирически проверено в этой же сессии (агент
   ...
   Это официально документированное поведение SQLAlchemy (паттерн
   `begin_nested()` + перехват `IntegrityError` конкурентного INSERT описан
   в её официальной документации по транзакциям), не догадка и не
   недокументированная деталь. Строка выше — не обязательная поправка
   корректности, а защита читаемости: без неё правильность конкретно ЭТОГО
   блока опирается на знание конкретного документированного, но
   неочевидного при беглом чтении поведения ORM — явный `flush()` делает
   точку возможного `IntegrityError` видимой прямо в коде, не только в
...
**Валидация при старте приложения.** Все три переменные проверяются на
непустое значение при импорте модуля `server/blacksea.py` (тем же способом,
что и остальная конфигурация модуля — на уровне констант, не лениво при
первом запросе): отсутствие любой → приложение падает при старте с понятной
ошибкой, а не тихо принимает вебхуки, которые не сможет обработать.
Это отличается от текущего NOWPayments-кода (`NP_API_KEY = os.environ.get(...,
"")` — молча становится пустой строкой, `payments.py:31-32`) — осознанное
ужесточение именно для нового платёжного контура, не бага в существующем
коде; менять поведение NOWPayments не входит в объём этой спеки.
...
async with db.begin_nested():
    ...
    if row.value != stale_token:
        ...
        try:
            access_token, refresh_token = await refresh_blacksea_token(...)  # timeout=15
        except (httpx.TimeoutException, httpx.ConnectError, httpx.HTTPError):
            raise  # см. ниже — исключение обязано выйти из begin_nested() как есть
        row.value = access_token
        # + обновить строку blacksea_refresh_token тем же способом
```

**Сетевая ошибка/таймаут именно у самого refresh-вызова** (не у последующей
верификации продажи) — исключение не перехватывается внутри
`async with db.begin_nested()`, а выходит из блока как есть: SQLAlchemy
откатывает savepoint (та же механика, что и `except IntegrityError` в п.6
потока — откат до savepoint, внешняя транзакция остаётся валидна), старые
`access_token`/`refresh_token` в `app_settings` НЕ меняются. Вызывающий код
(шаг 4 потока) ловит это исключение снаружи и обрабатывает как обычную
сетевую ошибку API BlackSea — 200 OK, лог ERROR, не начислять (тот же путь,
что «Сетевая ошибка/невалидный ответ» в шаге 4).

**Бюджет: не больше одного refresh и не больше одного повтора запроса
продажи на один входящий вебхук.** После успешного refresh повтор идёт со
свежим токеном; если и он снова даёт 401 — второй refresh с тем же входом
может лишь повторить тот же результат (значит проблема не в протухшем
токене, а в отозванном доступе или изменённом scope), и цикл обязан
остановиться, а не рефрешить бесконечно. Enforcement — флаг «refresh уже
выполнялся в рамках этого запроса» в области видимости обработчика; провал
повторного запроса после единственного refresh → лог ERROR + 200 OK, не
начислять.
...
**Эта ветка (весь блок `with_for_update()`) выполняется ТОЛЬКО в ответ на
401 от первого вызова `/api/v2/sales/{sale_id}`.** Обычный успешный запрос
(200 сразу) строку `blacksea_access_token` не блокирует и вообще её не
трогает — лок берётся исключительно на редком refresh-пути, не на каждом
вебхуке. Лок удерживается на всё время сетевого OAuth-round-trip к BlackSea
и до `await db.commit()` этой ветки (2-3 строки выше) — это осознанная
цена сериализации редкого пути, не бесконтрольное удержание до конца всей
обработки вебхука (начисление кредитов идёт уже в отдельной, следующей
транзакции — шаг 6 потока).

**Полный алгоритм retry, одним потоком, чтобы implementation не трактовал
по-своему:**
```text
GET /api/v2/sales/{sale_id} с текущим access_token
  200 → sale = ответ, идти к сверке (шаг 4 потока дальше)
  401 →
    SELECT app_settings.blacksea_access_token FOR UPDATE
    значение в БД уже != stale_token?
      да  → access_token = значение из БД (кто-то уже обновил)
      нет → refresh(refresh_token) → записать оба новых значения → commit
    повторить GET ровно ОДИН раз с этим access_token
      200 → sale = ответ, идти к сверке
      снова 401 или сетевая ошибка → 200 OK, лог ERROR, не начислять — цикл
      останавливается, второй refresh не запускается
```
В ветке «значение в БД уже != stale_token» читается и используется ТОЛЬКО
`access_token` — `refresh_token` в этой ветке не читается и не трогается,
обмен не запускается (обменивать нечего, токен уже свежий).
...
- Два одновременных вебхука получают 401 от BlackSea API одновременно
  ...
  `sale_id`) корректно начисляют. **На SQLite этот тест доказывает только
  алгоритм** (перечитывание значения перед refresh, бюджет «один refresh»)
  — реальную сериализацию через `FOR UPDATE` он не проверяет и не может
  (см. честную границу тестовой среды ниже); реальная блокировка — на
  PostgreSQL в проде, отдельным ручным прогоном перед боевым включением.
- Отсутствие/сломанный тип обязательного поля в ответе API (`paid` не bool,
  `email` отсутствует и т.п.) → не начислять, лог ERROR, 200 OK — покрывает
  fail-closed ветку п.4 потока.
- `quantity != "1"` в теле вебхука → не начислять автоматически, алерт на
  ручной разбор, 200 OK.
- Сетевая ошибка/таймаут именно у refresh-вызова (не у верификации продажи)
  → старые токены в `app_settings` не изменены, не начислять, 200 OK.
- Второй 401 подряд после единственного refresh (refresh не помог,
  токен реально невалиден/scope изменён) → второй refresh не запускается,
  не начислять, лог ERROR, 200 OK.
...
**Честная граница тестовой среды.** Тесты идут на in-memory SQLite
(`server/tests/conftest.py`). Диалект SQLite вырезает `FOR UPDATE`
(`sqlalchemy/dialects/sqlite/base.py` — `for_update_clause` возвращает
пустую строку) — тест на «конкурентный 401 → refresh ровно один раз»
на SQLite доказывает АЛГОРИТМ (перечитывание значения под локом, единственность
вызова refresh, бюджет повторов), но НЕ доказывает реальную блокировочную
семантику `with_for_update()` — она обеспечивается PostgreSQL в проде и
в этой тестовой среде не проверяется. Аналогично для блокировки строки
покупателя (`user.with_for_update()`). `UNIQUE(sale_id)` в SQLite работает
по-настоящему, поэтому тест на `IntegrityError`/однократное начисление
содержателен и на SQLite. Перед боевым включением — если возможность есть,
прогнать хотя бы вручную сценарий двух конкурентных вебхуков против
реальной PostgreSQL (staging/локальный Postgres), не полагаться только на
SQLite-тесты как на доказательство блокировки в проде.
