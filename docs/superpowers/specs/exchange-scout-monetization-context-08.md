# Биржа 2.0 (Exchange Scout) — ЧАСТЬ A-М (монетизация), файл 08: Context и Code Archaeology (2.6, 2.7, 2.8)
...
> Файлы Части A-М: **08 (этот файл)** · [09](exchange-scout-monetization-contracts-design-09.md) · [10](exchange-scout-monetization-adversarial-10.md) · [11](exchange-scout-monetization-tests-gate-11.md)
...
6. **Реакции GUI** (`main.py:3760-3767`), ровно три ветки: успех → обновить счётчик; `low_credits` →
   ...
   «offline_stopped». Обычный сетевой сбой без превышения 120 с возвращает клиенту `success=False`;
   клиент не подтверждает оплату и охоту не останавливает. При этом факт серверного списания может быть
   неизвестен, если запрос дошёл до сервера, а ответ потерялся (см. 2.8.6, A-16 спеки монетизации).
...
6. **Но long-poll и без побудки возвращает актуальный баланс.** `GET /vault/sync/{hwid}`
   ...
   **Вывод (проверенный, не предполагаемый):** расхождение локального счётчика с реальным балансом после
   потерянного ответа на списание обновится на одном из следующих циклов long-poll; ожидание одного цикла
   может составлять до ≈50 с, плюс время переподключения — без какого-либо уведомления со стороны
   `/use_credit`. Это выравнивает баланс на экране, но не сообщает, была ли оплачена конкретная находка.
...
3. **Rate limit 10 секунд на hwid: при попадании в окно сервер возвращает `success: True` с
   `note: "rate_limited"`, без публикации:**
   ...
   `server/roy.py:39`). Поэтому клиент 1.0, который смотрит только на `success` (`roy/roy_client.py:39`),
   считает такой репорт доставленным.
