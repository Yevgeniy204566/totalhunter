...
>
> **Claude прогоняет этот чек-лист в конце каждой крупной сессии/хангофа** (см. CLAUDE.md §7) и
> выдаёт владельцу краткий статус по зонам ниже. Это не замена внешнему 24/7-мониторингу — Claude
> не видит сервер между сессиями, если его не вызвали.

---

## 🚦 Зоны и действия (End-of-Session Check)

Выполнить последовательно на GCP: `free -h`, `df -h /`, `uptime`, `ps aux --sort=-%mem | head -10`,
`systemctl status totalhunter --no-pager`, `sudo -u postgres psql -c "SELECT pg_size_pretty(pg_database_size('totalhunter'));"`.

- **🟢 Зелёная зона:** диск < 80%, RAM used < 700MB (на e2-micro 1GB), load average < числа ядер,
  `totalhunter.service` active. *Действие:* ничего не делать, написать одну строку:
  `✅ Сервер в норме (RAM: X, Disk: Y%, DB: Z).`
- **🟡 Жёлтая зона:** диск 80–90%, или RAM 700–850MB, или есть зомби-процессы. *Действие:* выполнить
  `sudo /opt/totalhunter/cleanup.sh` вручную (помимо еженедельного cron), написать:
  `🛠 Выполнена автоочистка. Место/память освобождены.`
- **🔴 Красная зона:** диск > 90%, или RAM стабильно в swap, или load average стабильно высокий,
  или сервис не active. *Действие:* НЕ чинить самостоятельно тихо — сразу написать владельцу:
  `⚠️ КРИТИЧЕСКИЙ АЛЕРТ: [что именно]. Очистка не поможет / не помогла. Нужно решение (рестарт/апгрейд VM).`

Автоочистка на сервере (`/opt/totalhunter/cleanup.sh`, журналы 7д + pycache + /tmp + apt clean) висит
в `root` crontab каждое воскресенье 03:00 (`0 3 * * 0`) — работает сама, независимо от сессий Claude.
Установка идемпотентна (скрипт пересоздания крона сначала вычищает старую строку с тем же путём).
