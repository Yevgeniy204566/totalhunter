...
1. **Система РОЙ — сборка и полный тест** — собрать v1.2.9 с Roy модулем, протестировать OCR на живой бирже (Tesseract нужен в сборке)
2. **Реклама** — найти сеть с баннерами без высокого порога вывода (BitMedia $20 BTC, BidVertiser $10)
3. **Серверные миграции** — на GCP есть лишние миграции `14e8d8e2a95a_final_merge`, `575bdc292d9e_merge_heads`, `22864ea6408d_add_web_platform_tables` — нужно перенести их в репо
4. **DPI awareness** — отложено на v1.2.9. SetProcessDpiAwareness(2) должна быть первой строкой main.py до GUI импортов. Требует тест на 125%/150% DPI.
5. **Баг: бот выкидывает в магазин** — клиент жаловался, не диагностировали. Нужно выяснить при следующем случае.
