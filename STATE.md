...
> Последнее обновление: 2026-06-23 (Хангоф #104) — **Релиз v1.8.5 выпущен.** Фича «Древний» (Часть A) полностью реализована и задеплоена: калькулятор нормы в личном кабинете + интеграция `tournament_reader.py` в GUI бота (новая вкладка). Серия OCR-фиксов (DAWG, ВИП-значок, fuzzy-match имён, 2 краша). Уровни войск переименованы в игровой формат G/S/M («Состав»). Always-on-top теперь по умолчанию. Версия бота: **1.8.5**. Подробности → блок «Хангоф #104» ниже.
>
