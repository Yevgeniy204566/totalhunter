...
> Последнее обновление: 2026-06-03 **v1.6.9** — баг swing→ui_offsets устранён, D-Pad тюнинг сохраняется корректно.
...
| **Система РОЙ** | roy/ + server/roy.py + engine.py | ✅ **v1.6.8:** Кнопка «Обновить пул» удалена — пул обновляется через SSE (~1с от находки). Глобальный пул без фильтра по ГОСу. OCR K/X/Y универсален на всех языках. Event gate 144ч. | 2026-06-03 |
| **TG-канал тизер** | server/tg_channel.py | ⚠️ Задеплоен. `send_telegram_alert(percent)` — фоново через BackgroundTasks при новой бирже во время ивента. Картинка: `/opt/totalhunter/server/tg_teaser.jpg`. **НЕ ПРОВЕРЕН**: curl-тесты дают 404 (бот в канале @Total_Hunter — нужно перепроверить admin-права). Следующий живой тест — 07.06 20:00 Киев. | 2026-06-04 |
