...
> Последнее обновление: 2026-06-24 (Хангоф #105) — Дропдаун выбора сундука в личном кабинете теперь показывает полный эталонный список (139 видов, включая Sakura of Abundance), а не урезанное подмножество. Фикс читаемости имён лидеров на публичной таблице (letter-spacing + размер шрифта). End-of-Session проверка сервера выполнена впервые за несколько хангофов — диск был в жёлтой зоне (63%), автоочистка вручную → 58%. Релиз бота НЕ требовался (web+server only). Подробности → блок «Хангоф #105» ниже.
>
