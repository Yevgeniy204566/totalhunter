...
> Последнее обновление: 2026-06-19 — Backend-фундамент «Сундуки» **задеплоен на GCP** (коммит `a9d01f3`). `alembic upgrade head` дошёл до `h7c8e9s0t1c2`, `totalhunter.service` active (running), `/docs` → 200. По пути найдено и исправлено два независимых бага в истории миграций: (1) `p1q2r3s4t5u6_merge_all_heads.py` объявлял фиктивный two-parent merge — `n3o4p5q6r7s8` уже был потомком `14e8d8e2a95a`, не параллельной веткой; исправлено на одного родителя; (2) на проде в `alembic_version` залежались 2 лишние строки (`h4i5j6k7l8m9`, `k1g2s3t4a5t6`) от старого частичного прогона — удалены вручную SQL DELETE, оставлена только `n3o4p5q6r7s8`. Также обнаружено: `DATABASE_URL` с реальным паролем (`TotalHunter2026`) живёт в основном systemd-юните totalhunter.service (не в override.conf) — `systemctl show totalhunter -p Environment` показывает полную картину, `systemctl cat` — только override. **Следующий шаг:** собрать новый клиентский релиз (main.py/chest_reader.py изменились с последнего ZIP). Подробности → раздел «Сундуки» в таблице модулей ниже.
