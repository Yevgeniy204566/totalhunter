...
> Последнее обновление: 2026-06-18 **v1.7.3** — Хангоф #98. Модуль «Сундуки» (chest_reader.py + вкладка СУНДУКИ) реализован по TDD-плану (9 задач, subagent-driven), живой тест пройден (1276+ сундуков собрано, 0 крашей). Релиз НЕ собирался — версия осталась 1.7.3. Подробности → раздел «Сундуки» в таблице модулей ниже.
...
| **Сундуки** | chest_reader.py + main.py (вкладка СУНДУКИ) | ✅ **Реализовано и живо-протестировано (2026-06-18).** Без скролла — клик «Открыть» по верхней строке, список сам сдвигается. Имя отправителя и тип сундука читаются по ФИКСИРОВАННЫМ координатам (`SENDER_REF_RECT=(816,375,361,24)`, `SOURCE_REF_RECT=(865,398,371,24)`, через `coord_manager.to_region_dialog` — привязано к профилю калибровки), а не относительным кропом — после живого теста выяснилось что OCR «От:» нестабилен (От./Ot:/Or:), фикс-координаты убрали проблему полностью. **chest_type = строка «Источник:»** (например «Эпический отряд нежити»), а не общий заголовок «Сундук Эпического Монстра» — заголовок не различает источники. Локальный буфер `chest_buffer.db` (SQLite, gitignored) — пишет каждый сундук сразу, переживает краш/стоп/старт. Кнопка «Отправить на сервер» (auth.spend_credit hunt_type="chest") — **backend `/api/v1/chests/import` НЕ СУЩЕСТВУЕТ**, ожидаемо падает, данные остаются в SQLite. Живой тест: 1276+ сундуков, 0 крашей, имена/типы чистые (кроме редких единичных OCR-искажений букв в кириллице — отдельная, не системная проблема). Новая служебная утилита `coord_picker.py` (не часть бота) — живая лупа+координаты курсора, для калибровки таких фиксированных прямоугольников в будущем. | 2026-06-18 |
