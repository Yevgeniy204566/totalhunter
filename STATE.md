...
> Последнее обновление: 2026-06-22 (Хангоф #101) — **Сундуки: редизайн кабинета+публичной страницы, фикс дубля коллектора, полная геймификация сезона (3 спеки) задеплоены.** Релиз клиентского бота НЕ требуется — все изменения этой сессии веб-only (server/ + web/), деплоятся напрямую git push → GCP/Vercel, версия бота (`version.py`) остаётся 1.8.1. Подробности → раздел «Сундуки» ниже.
>
