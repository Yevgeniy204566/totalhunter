...
3. **Закрепление шапки публичной таблицы (Excel-style) — переоткрыть с нуля.** Технически реализовано и работало в этой сессии (раздельные `<table>` для шапки/тела + `position: sticky` на отдельной непойманной в скролл-ловушку обёртке + JS-синхронизация ширины колонок), но владелец отверг по визуальным ощущениям без уточнения деталей («плохо, откатывай как было»). **Перед повторной попыткой — обязательно спросить, что конкретно не понравилось** (дёргалось при скролле? сломалась синхронизация колонок? просто не зашло визуально?), не предполагать и не повторять ту же реализацию вслепую. Код решения сохранён в истории git (коммит `0548f9f`, отменён `4435341`) — можно поднять как референс.
