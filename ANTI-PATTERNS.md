...
**Правило:** Горизонтальный скролл-контейнер (`overflow-x: auto`) и `position: sticky`
относительно ВСЕЙ СТРАНИЦЫ — взаимоисключающие на одном и том же элементе по спецификации
CSS, это не баг браузера и не лечится переключением `overflow-y` на другое значение. Если
нужны ОБА эффекта одновременно — единственный надёжный путь — структурно разделить
прилипающий элемент и горизонтально скроллящийся элемент на разные узлы DOM (см. попытку v2
выше как референс в git, коммит `0548f9f`). Перед повторной попыткой подобного — обязательно
уточнить у владельца ТОЧНУЮ причину отказа, не предполагать и не переделывать вслепую.
